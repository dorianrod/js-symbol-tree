...
describe('move to item', () => {
        let tree;
        beforeEach(() => {
                tree = new SymbolTree();
                tree.appendChildrenRecursively(
                        {
                                children: [
                                        {
                                                name: 'a',
        ...
        it('move last to first', () => {
                const a = tree.getByPath('$0.0.0');
                const b = tree.getByPath('$0.0.1');
                const c = tree.getByPath('$0.0.2');
                tree.moveItemToIndex(c, 0);

                expect(tree.previousSibling(c)).toBeNull();
                expect(tree.nextSibling(c)).toBe(a);
                expect(tree.previousSibling(a)).toBe(c);
                expect(tree.nextSibling(a)).toBe(b);
                expect(tree.previousSibling(b)).toBe(a);
                expect(tree.nextSibling(b)).toBeNull();
        });

        it('move last to first when index < 0', () => {
                const a = tree.getByPath('$0.0.0');
                const b = tree.getByPath('$0.0.1');
                const c = tree.getByPath('$0.0.2');
                tree.moveItemToIndex(c, -5);
        ...
        it('move first to last', () => {
                ...
                const b = tree.getByPath('$0.0.1');
                const c = tree.getByPath('$0.0.2');
                tree.moveItemToIndex(a, 2);

                expect(tree.previousSibling(b)).toBeNull();
                expect(tree.nextSibling(b)).toBe(c);
                expect(tree.previousSibling(c)).toBe(b);
                expect(tree.nextSibling(c)).toBe(a);
                expect(tree.previousSibling(a)).toBe(c);
                expect(tree.nextSibling(a)).toBeNull();
        });

        it('move first to last when index > length', () => {
                const a = tree.getByPath('$0.0.0');
                const b = tree.getByPath('$0.0.1');
                const c = tree.getByPath('$0.0.2');
                tree.moveItemToIndex(a, 1000);
        ...
        it('move first to middle', () => {
                const a = tree.getByPath('$0.0.0');
                const b = tree.getByPath('$0.0.1');
                const c = tree.getByPath('$0.0.2');
                tree.moveItemToIndex(c, 1);

                expect(tree.previousSibling(a)).toBeNull();
                expect(tree.nextSibling(a)).toBe(c);
                expect(tree.previousSibling(c)).toBe(a);
                expect(tree.nextSibling(c)).toBe(b);
                expect(tree.previousSibling(b)).toBe(c);
                expect(tree.nextSibling(b)).toBeNull();
        });
});
